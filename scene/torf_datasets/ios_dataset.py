...
from scene.torf_datasets.my_utils.my_utils import (
    rgb2gray,
    resize_all_images,
    tof_from_depth,
)
...
class IOSDataset(ToFDataset):
    def __init__(
        self,
        args,
        file_endings={"tof": "npy", "color": "npy", "depth": "npy", "cams": "npy"},
    ):
        super().__init__(args, file_endings)

    def _read_tof(self, tof_filename):
        return np.zeros(
            [self.args.tof_image_height, self.args.tof_image_width, 3], dtype=np.float32
        )
