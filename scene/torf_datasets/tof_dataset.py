...
from scene.torf_datasets.my_utils.my_utils import normalize_im_max, depth_from_tof
from scene.torf_datasets.my_utils.camera_utils import (
    crop_mvs_input,
    scale_mvs_input,
    get_extrinsics,
    get_camera_params,
...
class ToFDataset(object):
    ...
    def _get_tof_filename(self, frame_id):
        tof_filename = os.path.join(
            self.args.source_path, f'tof/{frame_id:04d}.{self.file_endings["tof"]}'
        )
    ...
    def _get_color_filename(self, frame_id):
        color_filename = os.path.join(
            self.args.source_path, f'color/{frame_id:04d}.{self.file_endings["color"]}'
        )
    ...
    def _get_depth_filename(self, frame_id):
        depth_filename = os.path.join(
            self.args.source_path, f'depth/{frame_id:04d}.{self.file_endings["depth"]}'
        )
    ...
    def _process_camera_params(self, args):
        self.tof_intrinsics, self.tof_extrinsics = get_camera_params(
            os.path.join(
                args.source_path, f'cams/tof_intrinsics.{self.file_endings["cams"]}'
            ),
        ...
        self.color_intrinsics, self.color_extrinsics = get_camera_params(
            os.path.join(
                args.source_path, f'cams/color_intrinsics.{self.file_endings["cams"]}'
            ),
    ...
    def _create_splits(self, args):
        ...
        self.i_train = np.array(
            [
                ...
                if (i not in self.i_test and i not in self.i_val) or (1 == 1)
            ]
        )
    ...
    def get_batch(
        self, i_train, N_rand, H, W, args, outputs=["tof_images", "color_images"]
    ):
        ...
        for k in outputs:
            if self.dataset[k][img_i] is not None:
                batch[k] = tf.gather_nd(self.dataset[k][img_i], select_inds)
            else:
                batch[k] = None
