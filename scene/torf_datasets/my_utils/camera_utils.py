...
from scene.torf_datasets.my_utils.sampling_utils import *
...
def scale_mvs_input(images, cams, view_num, depths=None, scale=1):
    ...
    for view in range(view_num):
        ...
        if not depths is None:
            depths[view] = scale_image(
                depths[view], scale=scale, interpolation=cv2.INTER_NEAREST
            )
        ...
        if mode == "training":
            ...
            for j in range(0, 7):
                ...
                for p in range(0, int(cluster_list[0])):
                    ...
                    ref_cam_path = os.path.join(
                        cam_folder, ("%08d_cam.txt" % ref_index)
                    )
...
def gen_blendedmvs_path(blendedmvs_data_folder, mode="training_mvs"):
    ...
    if mode == "training_mvs":
        proj_list = (
            open(os.path.join(blendedmvs_data_folder, "BlendedMVS_training.txt"))
            .read()
            .splitlines()
        )
    elif mode == "training_mvg":
        proj_list = (
            open(os.path.join(blendedmvs_data_folder, "BlendedMVG_training.txt"))
            .read()
            .splitlines()
        )
    elif mode == "validation":
        proj_list = (
            open(os.path.join(blendedmvs_data_folder, "validation_list.txt"))
            .read()
            .splitlines()
        )
    ...
    for data_name in proj_list:
        ...
        for idx in range(0, image_num):
            ...
            ref_image_path = os.path.join(
                dataset_folder, "blended_images", "%08d_masked.jpg" % ref_idx
            )
            ref_depth_path = os.path.join(
                dataset_folder, "rendered_depth_maps", "%08d.pfm" % ref_idx
            )
            ref_cam_path = os.path.join(
                dataset_folder, "cams", "%08d_cam.txt" % ref_idx
            )
            paths.append(ref_image_path)
            paths.append(ref_cam_path)
            ...
            for cidx in range(0, FLAGS.view_num - 1):
                ...
                view_image_path = os.path.join(
                    dataset_folder, "blended_images", "%08d_masked.jpg" % view_idx
                )
                views_depth_path = os.path.join(
                    dataset_folder, "rendered_depth_maps", "%08d.pfm" % view_idx
                )
                view_cam_path = os.path.join(
                    dataset_folder, "cams", "%08d_cam.txt" % view_idx
                )
                paths.append(view_image_path)
                paths.append(view_cam_path)
...
def get_extrinsics(render_extrinsics_file, args, default_exts=None):
    ...
    if os.path.exists(render_extrinsics_file):
        ...
        if len(extrinsics_data.shape) == 2:
            extrinsics_data = np.array([extrinsics_data])
    else:
...
def reprojection_test(image, ray_gen_fn, project_fn, relative_pose, distance):
    ...
    interp_image = interpolate_image(
        image[None], pixels[None], shifts_x=[0], shifts_y=[0]
    )
